...
Rseg.prototype.process = function (c, is_last) {
  this.englishProcess(c, is_last);
  this.dictProcess(c, is_last);
}

Rseg.prototype.englishProcess = function (c, is_last) {
  var nomatch = true,
      tmpWord = '';
  
  if(this.englishEngine.is_running()){
    var EMatchAndWord = this.englishEngine.process(c, is_last);
    tmpWord = EMatchAndWord[1];
    if (EMatchAndWord[0]) {
      nomatch = false;
    }else{
      if (this.englishDictionary.indexOf(tmpWord.toLocaleLowerCase()) == -1) {
        tmpWord = '';
      };
